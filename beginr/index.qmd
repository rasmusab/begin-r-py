...
Friends of `+`, like `-`, `/`, and `*` works like expected. R also includes a large number of mathematical functions, like `log()`, `exp()`, and `sqrt()`. To _call_ a function you write its name followed by the _argument_ between parentheses, like `log(2.72)` or `sin(1.57)`.
...
And, of course, there are functions that takes both numbers and strings as arguments. But the order of the arguments is important! For example, for `rep()` the first argument is the value to repeat, and the second argument is the number of repetitions. 
...
if(is.numeric(.result) && grepl("max", .user_code) && .result == max(temp)) {
  list(correct = TRUE, message = "Hot! And that's 108.3 F for you Farenheiters.")
} else {
...
<div class="task-callout">Use, you guest it, `mean()` to get the mean/average of `temp` </div>
...
Many functions in R can be given _optional_ arguments, and `plot()` has many that changes the appearance of the plot. Arguments to a function can also be given in the form `the_argument_name = "the value"`, and then the argument order doesn't matter anymore.
