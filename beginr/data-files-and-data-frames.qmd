...
We could stop here, we already have a quite nice summary of our ice cream sames. But let's tack on one more thing to out data analysis pipeline.
...
Here we wrapped `temp_max` with `desc(temp_max)` to have sort in `desc`ending order, otherwise the defulat is to sort in ascending order.
...
if(
  ...
  list(correct = TRUE, message = "It's clear we sell the most during the weekend.")
} else {
